...
import {bindEvent, close_on_clickOutside, createElement, filter_box, save_confirm} from "../helpers/basics";
import {Site} from "../classes/site";
import {FILE_ADMIN, FILE_RESPONSES} from "../variables/urls";
...
export const DataLoader = {
	...
	init: function() {
		...
		return Site.studies.init().then(() => {
			let study = Site.studies.get_current();
			
			if(study.version() === 0)
				return Promise.resolve(this.box);
			return PromiseCache.loadJson(FILE_ADMIN+"?type=list_data", (backups) => {
				let groups = study.groups();
				let groupIndex = {};
				for(let i=groups.length-1; i>=0; --i) {
					let group = groups[i];
					groupIndex[group.internalId()] = group.name();
				}
				...
				for(let i=backups.length-1; i>=0; --i) {
					let entry = backups[i];
					if(groupIndex.hasOwnProperty(entry))
						currentList.push([groupIndex[entry], entry]);
					else {
						let [date, internalId] = this.get_backupTitle(entry);
						if(internalId === -1 || !groupIndex.hasOwnProperty(internalId))
							backupsList.push([entry, entry]);
						else
							backupsList.push([date + " " + groupIndex[internalId], entry]);
					}
				}
				...
				this.box.current(currentList);
				this.box.backups(backupsList);
				
				return this.box;
			}, "post", "study_id="+study.id());
		});
	},
	reload: function() {
	...
	empty_data: function() {
		...
		if(!Site.admin.is_loggedIn() || !save_confirm(Lang.get("confirm_delete_data").replace("%", study.title())))
			return;
		
		Site.load_withState(FILE_ADMIN + "?type=empty_data", false, "post", "study_id="+study).then(() => {
			this.reload();
		});
	},
	load_data: function(fileTitle, fileName, titleObservable) {
		let studyId = Site.studies.get_currentId();
		
		let url = FILE_RESPONSES.replace('%1', studyId).replace('%2', fileName);
		...
		PromiseCache.loadCSV(url).then((loader) => {
				...
				return loader.index_data_async(bottomIndex).then(function() {
					...
					for(let min=Math.max(i-max_elements_displayed+1, 0); i>=min; --i) {
						...
						bindEvent(tr, "click", function(e) {
							...
							if(selectedRows) {
								selectedRowCount_el.innerText = Lang.get("selected_rows_x").replace("%d", selectedRows);
			...
			let update_table = function() {
				...
				titleObservable(fileTitle + " (" + Lang.get("colon_entry_count") + " " + loader.rows_count + ")");
				return promise;
			};
			...
			for(let i=0, max=header_names.length; i<max; ++i) {
				...
				bindEvent(span, "click", function(e) {
					...
					loader.index_data_async().then(function() {
						...
						label.appendChild(createElement("span", false, {innerText: Lang.get("toggle_all").replace("%", values.length)}));
