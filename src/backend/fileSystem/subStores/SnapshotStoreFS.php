...
class SnapshotStoreFS implements SnapshotStore {
    ...
    public function createSnapshot(){
        ...
        try {
            ...
            $exclude = [realpath(PathsFS::folderSnapshot()), realpath(PathsFS::folderDataBackup())];
...
class DirFilter extends RecursiveFilterIterator {
    ...
    public function accept(): bool {
        return !($this->current()->isDir() && in_array($this->current()->getFilename(), $this->exclude));
    }
    public function getChildren(): null | RecursiveFilterIterator {
        $innerIterator = $this->getInnerIterator();
        if($innerIterator instanceof RecursiveIterator) {
            return new DirFilter($innerIterator->getChildren(), $this->exclude);
        }
        return null;
    }
}