...
class Permission {
	 ...
	 * @throws Exception
	 */
	static function create_readPermission_htaccessFile($study_id, $permissions=null) {
		if($permissions == null)
		...
		$file = Files::get_folder_responses($study_id).FILENAME_HTACCESS;
		if(!file_put_contents($file, sprintf(HTACCESS_RESPONSES_TEMPLATE, $s), LOCK_EX))
			throw new Exception('Could not secure ' .Files::get_folder_responses($study_id) .'! All study data may be reachable without password!');
		else {
			chmod($file, 0666);
			return true;
		}
