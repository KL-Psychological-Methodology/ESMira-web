...
class SaveStudy extends HasWritePermission {
	...
	private function extractObservedEntriesFromChart(stdClass $chart, stdClass $currentObservedVariables, array &$currentIndex) {
		...
		foreach($chart->axisContainer ?? [] as $axisContainer) {
			$this->extractObservedEntryFromAxis(
				$axisContainer->yAxis ?? new stdClass(),
				$currentIndex,
				$currentObservedVariables,
				$storageType,
				$timeInterval
			);
			...
			if($dataType == CreateDataSet::STATISTICS_DATATYPES_XY) {
				$this->extractObservedEntryFromAxis(
					$axisContainer->xAxis ?? new stdClass(),
					$currentIndex,
					$currentObservedVariables,
					$storageType,
					$timeInterval
		...
		if(isset($chart->displayPublicVariable) && $chart->displayPublicVariable) {
			foreach($chart->publicVariables as $axis) {
				$this->extractObservedEntryFromAxis(
					$axis->yAxis ?? new stdClass(),
					$this->publicObservedIndex,
					$this->publicObservedVariables,
					$storageType,
					$timeInterval
				);
				if($dataType == CreateDataSet::STATISTICS_DATATYPES_XY) {
					$this->extractObservedEntryFromAxis(
						$axis->xAxis ?? new stdClass(),
						$this->publicObservedIndex,
						$this->publicObservedVariables,
						$storageType,
						$timeInterval
	...
	function exec(): array {
		...
		// Forward to plugins
		//
		Configs::getDataStore()->getPluginStore()->handleAdminSaveStudy($this->studyCollection);
