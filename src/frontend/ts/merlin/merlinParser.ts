...
export class MerlinParser {
	...
	private assignmentStatement(): Stmt {
		...
		if (this.match([
			...
			const assignmentOperator = this.previous()
			let value = this.expression()

			if (assignmentOperator.type != MerlinTokenType.EQUAL) {
				let arithmeticType = null;
				switch (assignmentOperator.type) {
					case MerlinTokenType.PLUS_EQUAL: arithmeticType = MerlinTokenType.PLUS; break;
					case MerlinTokenType.MINUS_EQUAL: arithmeticType = MerlinTokenType.MINUS; break;
					case MerlinTokenType.STAR_EQUAL: arithmeticType = MerlinTokenType.STAR; break;
					case MerlinTokenType.SLASH_EQUAL: arithmeticType = MerlinTokenType.SLASH; break;
					case MerlinTokenType.MODULO_EQUAL: arithmeticType = MerlinTokenType.MODULO; break;
					default: throw new ParseError(assignmentOperator, "Error: Used compound assignment operator is not implemented (this should be unreachable)")
				}
				const arithmeticOperator = new MerlinToken(arithmeticType, assignmentOperator.lexeme, assignmentOperator.line)
				value = new BinaryExpr(expr, arithmeticOperator, value)
			}
			...
			if (expr instanceof VariableExpr) {
				...
				throw new ParseError(assignmentOperator, "Invalid assignment target.")
			}
		}
	...
	private factor(): Expr {
		...
		while (this.match([MerlinTokenType.SLASH, MerlinTokenType.STAR, MerlinTokenType.MODULO])) {
			const operator = this.previous()
